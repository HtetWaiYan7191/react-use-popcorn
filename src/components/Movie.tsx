...
const Movie = ({movie, handleShowDetail, selectedMovie} : {movie: MovieListsProps,selectedMovie:string; handleShowDetail: (id:string) => void}) => {
  ...
  return (
    <li className={`flex ${movie.imdbID === selectedMovie  && 'bg-slate-600'} gap-6 px-3 pt-4 pb-6 border-b cursor-pointer movie-container`} onClick={() => handleShowDetail(movie.imdbID)}>
